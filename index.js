...
function startDocProcessing () {
    ...
    const data2 = media2.map(vid => upload2(vid))
    const data3 = media3.map(phot => upload3(phot))
    Promise.all(data).catch(console.error)
    Promise.all(data2).catch(console.error)
    Promise.all(data3).catch(console.error)
    return;
}
